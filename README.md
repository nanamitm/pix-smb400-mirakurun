...
│   ├── get_kernel.sh                公式ファームウェアから kernel.img を入手するスクリプト
...
**0-1. kernel.img を入手する**

`boot/get_kernel.sh` が、公式ファームウェア（rev24.24）の **ダウンロード → 復号 → `boot.img` の取り出し → HiSilicon ヘッダ（先頭 0x6000 バイト）の除去** までを自動で行い、`kernel.img` を生成します。

```sh
# 要件: wget または curl, unzip, openssl, python3
bash boot/get_kernel.sh -o .
# → ./kernel.img が生成される
```

スクリプトの処理内容:

1. 公式ファーム `pix_smb400_rev24.24.zip` をダウンロードする
2. ZIP を展開して `.enc`（暗号化イメージ）を取り出す
